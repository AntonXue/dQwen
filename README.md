...
that does not exist yet; see `dqeval/families/sdar.py`.)
...
from dqeval.sampler import DecodeConfig, generate
...
compat shims total (the compat sections of `dqeval/families/*.py`).
...
forwards, raises nothing, and returns noise. `dqeval/adapter.py::assert_finite_rope()`
