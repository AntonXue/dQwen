"""Diffusion LM from Qwen3; the BOS/logit shift preserves the AR next-token bias."""
...
# Qwen3 tokenizer ids
MASK_TOKEN_ID = 151660  # <|fim_middle|>
PAD_TOKEN_ID = 151643   # <|endoftext|>
BOS_TOKEN_ID = 151652   # <|vision_start|>
...
class DiffuQwen3Config(Qwen3Config):
    model_type = "diffuqwen3"


class DiffuQwen3(Qwen3ForCausalLM):
    config_class = DiffuQwen3Config

    def __init__(self, config: DiffuQwen3Config):
    ...
    def from_qwen3(
        ...
        """Seed from a pretrained causal Qwen3."""
        ...
        cfg = DiffuQwen3Config(**qwen.config.to_dict())
        cfg._attn_implementation = "sdpa"
        model = cls(cfg)
        model.load_state_dict(qwen.state_dict(), strict=False)

        # Re-tie lm_head to embed_tokens if the source ties.
    ...
    # Alias so train_stage1.py (which calls .from_qwen3_5) still works.
    from_qwen3_5 = from_qwen3


# Ensures that save_pretrained emits auto_map and copies this file.
DiffuQwen3Config.register_for_auto_class()
DiffuQwen3.register_for_auto_class("AutoModel")
